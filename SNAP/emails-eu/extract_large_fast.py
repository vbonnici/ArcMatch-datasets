...
i_o_prefix = sys.argv[6]			#path... odir/nome_*  0.gfd, 1.gfd
...
#    i_nof_edges = math.ceil(i_perc_edges * ((i_nof_nodes * (i_nof_nodes - 1)) / 2))
        i_perc_edges = (i_nof_nodes - 1) / ((i_nof_nodes * (i_nof_nodes )) / 2)
    i_nof_edges = math.ceil(i_perc_edges * ((i_nof_nodes * (i_nof_nodes )) / 2))
...
def extract_subgraph(inet, available_nodes, name, nof_nodes, nof_edges, is_directed):
    ...
    bone_parents =dict()
    for n in inet.get_neighs(s_nodes[0]):
        if inet.is_edge(s_nodes[0],n):
            bone_parents[n] = (s_nodes[0], True)
        else:
            bone_parents[n] = (s_nodes[0],False)
    s_edges = set()
    ...
    for i in range(nof_nodes - 1):
        ...
        if bone_parents[a][1]:
            s_edges.add( (bone_parents[a][0], a) )
        else:
            s_edges.add( (a, bone_parents[a][0]) )
        ...
        for n in inet.get_neighs(a):
            if (n not in s_nodes) and (n not in s_neighs):
                ...
                bone_parents[n] = a
                if inet.is_edge(a,n):
                    bone_parents[n] = (a, True)
                else:
                    bone_parents[n] = (a,False)
        #print(s_nodes, s_neighs)
    ...
    for i in range(len(s_nodes)):
        ...
        p = None
        for p in inet.get_neighs(s_nodes[i]):
            if p != s_nodes[i]:
                break
        if p == None:
